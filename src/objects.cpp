...
std::pair<Itsct_stat, Vec3<double>> Sphere::first_intersection(
    Vec3<double> pos, Vec3<double> angle) const {
  ...
  if (small_root < eps) {
    ...
    return std::make_pair(Itsct_stat::HAS_INTERSECTION,
                          this->center + pos + angle * big_root);
  } else {
    return std::make_pair(Itsct_stat::HAS_INTERSECTION,
                          this->center + pos + angle * small_root);
  }
}
