...
RGB ray_trace(Vec3<double> camera_pos, Vec3<double> camera_angle,
  ...
  for (Object *obj : objs) {
    ...
    if (closest == NULL ||
        distance(camera_pos, point) < distance(camera_pos, intersection)) {
      closest = obj;
      intersection = point;
    }
  }
  if (closest == NULL) {
    ...
    for (Light light : lights) {
      ...
      total_intensity +=
          std::max(0., dot(light_dir, norm_vec)) * light.intensity;
    }
...
Return a 2D color array which represents the rendered image. Index i, j
represents the pixel at row i (from top to bottom) and column j (from left to
right).
*/
...
vector<vector<RGB>> render(Vec3<double> camera_pos, Vec3<double> camera_angle,
  ...
  const Vec3<double> down =
      Vec3<double>(camera_angle.z / _ * camera_angle.x,
                   camera_angle.z / _ * camera_angle.y, -_);
